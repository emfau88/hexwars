...
import { cellKey, parseCellKey } from '../core/hex';
...
export function updateMovements(
  ...
  for (let index = armies.length - 1; index >= 0; index -= 1) {
    ...
    if (army.pathIndex < army.path.length - 1) {
      ...
      if (army.kind === 'supply' && (!reached || !next || reached.owner !== army.owner || next.owner !== army.owner)) {
        onArrival(army, reached ?? null);
