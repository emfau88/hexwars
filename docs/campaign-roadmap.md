...
| 11 | Partial | Level 1 keeps its complete 7×13 raster but uses only 12 playable cells. Its bases are six moves apart and five optional lateral cells add choice without extending the mandatory path. The opponent retains normal behavior and equal starting strength; only its Level-1 production is about 16% slower. The automated pacing smoke resolves in roughly 27 seconds. Level 2 is rebuilt; Level 3, mastery goals and human acquisition testing remain. |
...
- Level 1 keeps the full decorative raster while using only 12 playable cells and a six-move direct route. Its AI remains active with equal starting strength; only enemy production is subtly reduced for this level.
